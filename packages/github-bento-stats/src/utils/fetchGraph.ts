...
export const fetchUserGraph = async (username: string): Promise<{ graph: string }> => {
  try {
    ...
    const adjustedContributions = new Array(shiftDays).fill({ date: "", contributionCount: 0 }).concat(currentYearContributions);
